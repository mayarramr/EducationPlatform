...
const courses = [
    { icon: <HiOutlineBookOpen className="small-font me-1" />, bgColor: '#E1E2F6', color: '#5855A9', title: 'Basic : HTML and CSS', stats: [{ icon: <HiOutlineBookOpen className="small-font me-1" />, value: 22 }, { icon: <FaRegClipboard className="small-font me-1" />, value: 8 }, { icon: <IoPeople className="small-font me-1" />, value: 99 },] },
    { icon: <i className="fa-solid fa-share-nodes" style={{ color: '#FF965C' }}></i>, bgColor: '#F8EFE2', title: 'Branding Design', stats: [{ icon: <HiOutlineBookOpen className="small-font me-1" />, value: 22 }, { icon: <FaRegClipboard className="small-font me-1" />, value: 8 }, { icon: <IoPeople className="small-font me-1" />, value: 99 },] },
    {icon: <PiWaveSawtoothBold style={{ color: '#559E3B' }} />, bgColor: '#EFF7E2', title: 'Motion Design', stats: [{ icon: <HiOutlineBookOpen className="small-font me-1" />, value: 22 }, { icon: <FaRegClipboard className="small-font me-1" />, value: 8 }, { icon: <IoPeople className="small-font me-1" />, value: 99 },
    ]
    },];
        ...
        <>
            ...
            <div className={`row justify-content-evenly my-3 ${isScreenSmall ? 'gy-3' : ''}`}>
                {courses.map((course, index) => (
                    <div key={index} className={`col-md-3 py-4 px-1 rounded-4 ${isScreenSmall ? 'w-75' : ''}`} style={{ backgroundColor: course.bgColor }}>
                        <div className="bg-white rounded-pill d-flex align-items-center justify-content-center" style={{ width: '30px', height: '30px' }}>{course.icon}</div>
                        <p className={`fw-bold my-3 ${isScreenSmall ? 'ms-2' : 'text-center'}`}>{course.title}</p>
                        <div className="d-flex py-1 rounded-2 justify-content-evenly" style={{ backgroundColor: '#FCF9FF' }}>
                            {course.stats.map((stat, idx) => (
                                <div key={idx} className="d-flex align-items-center">
                                    {stat.icon}
                                    <p className="font-sm fw-bold">{stat.value}</p>
                                </div>
                            ))}
                        </div>
                    </div>
